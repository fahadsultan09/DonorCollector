...
class CreateFunds2 extends StatefulWidget {
  CreateFunds2(this.user);
  User user;
  @override
  _CreateFunds2State createState() => _CreateFunds2State();
}
    ...
    if (validateAndSave()) {
      try {
        ...
        if (_user != null) {
          ...
          _firestore
              .collection("Users")
              .document(widget.user.fatherName)
              .setData({"1": 1});
          _firestore
              .collection("Users")
              .document(widget.user.fatherName)
                ...
                child: Column(
                  children: <Widget>[
                    ...
                    TextFormField(
                      keyboardType: TextInputType.text,
                      validator: (input) => input.isEmpty
                          ? 'Source of Income cannot be empty'
                          : null,
                      onChanged: (value) {
                    ...
                    TextFormField(
                      keyboardType: TextInputType.text,
                      validator: (input) =>
                          input.isEmpty ? 'Skills cannot be empty' : null,
                      onChanged: (value) {
                    ...
                    TextFormField(
                      keyboardType: TextInputType.text,
                      validator: (input) =>
                          input.isEmpty ? 'Education cannot be empty' : null,
                      onChanged: (value) {
